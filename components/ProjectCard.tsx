    ...
    return (
      <article className="group my-24 flex w-full flex-col items-start space-y-5 sm:mb-32">
        ...
        <Link href={`/${project.slug}`} className="block self-start">
          <h2 className="text-left text-3xl leading-none font-black tracking-tight whitespace-nowrap uppercase sm:text-4xl md:text-5xl">
            {project.title}
          </h2>
        </Link>
    ...
    return (
      ...
      >
        ...
        <div className="flex w-full flex-col items-start justify-center space-y-5 sm:w-2/5">
          <Link href={`/${project.slug}`} className="block self-start">
            <h2 className="inline-block text-3xl leading-none font-black tracking-tight whitespace-nowrap uppercase sm:text-4xl md:text-5xl">
              {project.title}
            </h2>
          </Link>
  ...
  if (isTextProject) {
    return (
      <article className="my-24 flex flex-col space-y-6 sm:mb-32">
        <Link href={`/${project.slug}`} className="block self-start">
          <h2 className="text-3xl leading-none font-black tracking-tight whitespace-nowrap uppercase sm:text-4xl md:text-5xl">
            {project.title}
          </h2>
        </Link>
